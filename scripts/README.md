...
2. 不足しているキーをファイルごと、言語ごとにグループ化
3. Gemini APIに翻訳とファイル編集を1回のリクエストで依頼（ファイルごと）
4. ファイルを更新（フォーマット保持）

**最適化**: 翻訳とファイル編集を1つのLLMコールで実行することで、API呼び出し回数を最小化しています（最大4回：ja.ts、en.ts、ja_notify.ts、en_notify.ts用）
...
スクリプトには以下のエラー処理機能が実装されています：

- **レート制限対応**: API使用量制限（429エラー）に達した場合、自動的にリトライします（最大3回）
- **リクエスト間隔**: 翻訳リクエスト間に1秒の遅延を挿入し、レート制限を回避
- **エラーメッセージ**: 問題が発生した場合、分かりやすいエラーメッセージを表示

レート制限に達した場合は、しばらく待ってから再実行してください。
...
- 翻訳生成にはAPIコストが発生します
- Gemini API無料枠には以下の制限があります：
  - リクエスト数: 1分あたり15リクエスト、1日あたり1500リクエスト
  - 制限に達した場合は自動リトライしますが、最大3回まで
