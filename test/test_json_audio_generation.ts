...
async function testJSONAudioGeneration(): Promise<void> {
  ...
  try {
    ...
    try {
      await page.waitForSelector('[data-testid="preview-content"] img, [data-testid="preview-content"] video, .preview-area img, .preview-area video', {
        timeout: 5000
      });
    ...
    const completedChecks = [completionNotificationFound, generateButtonReEnabled, previewContentFound, generationComplete].filter(Boolean).length;
    ...
    try {
      ...
      await page.waitForSelector('button:has-text("Play"), button[aria-label="Play"], button[title*="Play"]', { timeout: 5000 });
    ...
    console.log(`Project "${projectTitle}" was created, JSON was pasted, content was generated, and audio playback was started.`);
  } catch (error: unknown) {
