...
- ❌ 内部 schema 名: `imageParams`, `movieParams`, `audioParams.concurrency`, `speechParams.speakers` 等
- ❌ UI コンポーネント名: `Collapsible カード`, `Input type="number"`, `Switch コンポーネント`, `Select` 等
- ❌ フレームワーク・ライブラリ用語: `GraphAI`, `Pinia`, `Reka UI`, `Vue` 等
- ❌ 抽象的すぎる仕組み表現: 「動的ロード」「上流ライブラリの更新により」「ドロップダウンに自動反映」等

→ 「設定画面」「プロジェクトの Style タブ」「画像生成モデル」のように **ユーザーが実際に画面で見る言葉**で書く。

**D. カテゴリ分類の判断基準（追加）**

- **OEM ブランディング / ビルド設定 / 配布向け機能** → 開発者向け（一般ユーザーは触らない）
- **JSON 直接編集時のエラー表示改善** → 開発者向け（JSON 編集する人は上級ユーザー）
- **CLI 側で機能追加されたが app 側に UI 対応がないもの** → 載せない（上述「記載しない」ルールの再強調）
- **副次的な対応**（例: 提供元がモデルを deprecate → onboarding のデフォルト変更）は **独立セクションを増やさず、根本原因の項目に併記**する
